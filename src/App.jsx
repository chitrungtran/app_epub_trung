...
export default function App() {
  ...
  // --- LOGIC LOAD SÁCH (SỬA LỖI TREO APP) ---
  useEffect(() => {
    if (isReady && viewerRef.current) {
      ...
      const loadBook = async () => {
        try {
          ...
          setLoadingStep('Đang giải mã sách...');
          ...
          // --- CÁCH TIÊM FONT AN TOÀN HƠN ---
          // Thay vì chọc ngoáy sâu, ta dùng try-catch cho riêng phần này
          try {
             ...
             console.warn("Không tiêm được font, dùng font mặc định:", hookError);
          }

          setRendition(newRendition);
          
          setLoadingStep('Đang hiển thị...');
          ...
          setError(`Không đọc được sách: ${err.message}`);
        } finally {
          // --- QUAN TRỌNG: LUÔN LUÔN TẮT LOADING ---
          // Dù lỗi hay không cũng phải tắt để hiện giao diện
          setLoading(false);
          setLoadingStep(''); 
        }
      };
  ...
  const updateBookStyles = (rend, settings) => {
    ...
    try {
      ...
      // Force update nhẹ nhàng
      const location = rend.currentLocation();
      if (location && location.start) {
         // Không gọi display lại toàn bộ để tránh lag, chỉ cần themes update là đủ
         // EpubJS tự handle việc này
      }
    } catch (e) {
      console.warn("Lỗi cập nhật giao diện:", e);
    }
  };
