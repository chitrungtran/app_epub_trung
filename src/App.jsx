  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-hidden font-sans transition-colors duration-300" style={{ backgroundColor: prefs.bgColor, color: prefs.textColor }}>
      ...
      <div className="flex-none h-14 px-4 flex items-center justify-between border-b border-gray-400/20 backdrop-blur-sm z-50 relative">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { setShowToc(!showToc); setShowSettings(false); }} className={`p-2 rounded-full transition-colors ${showToc ? 'bg-teal-100 text-teal-800' : 'hover:bg-gray-400/20'}`} title="Mục lục"><List size={20} /></button>
          <button onClick={toggleTheme} className="p-2 rounded-full hover:bg-gray-400/20 transition-colors">{prefs.themeMode === 'dark' ? <Sun size={20}/> : <Moon size={20}/>}</button>
          <button onClick={() => { setShowSettings(!showSettings); setShowToc(false); }} className={`p-2 rounded-full transition-colors ${showSettings ? 'bg-teal-100 text-teal-800' : 'hover:bg-gray-400/20'}`}><Settings size={20} /></button>
          <button onClick={toggleFullscreen} className="p-2 rounded-full hover:bg-gray-400/20 transition-colors hidden sm:block"><Minimize size={20}/></button>
        </div>
      </div>
      ...
      {/* KHUNG ĐỌC SÁCH (Có thêm padding-bottom để không bị thanh status che mất chữ) */}
      ...
      {/* --- THANH TRẠNG THÁI (STATUS BAR) - MỚI --- */}
      {book && !loading && !error && (
      ...
      {/* Status Bar Desktop (Hiện khi màn hình to) */}
      <div className="hidden md:flex fixed bottom-0 left-0 right-0 h-8 bg-white/90 backdrop-blur border-t border-gray-200 items-center justify-between px-6 text-xs text-gray-500 z-50">
         ...
         <div className="flex items-center gap-2">
            <span>Tiến độ:</span>
            <span className="font-mono font-bold text-teal-700">{progress}%</span>
         </div>
         {/* Thanh tiến độ nền */}
